#DGP #2: Multivariate test: MSE and CI, non-exact tests
beta=1
N=200
...
#Computes nus
#Note:
#We have: nu_i=solve((X'X)_[1+i,1+i])
#Given that VarMat=(X'X)/(N-1)
#We use: nu_i=solve(VarMat[1+i,1+i])[1,1]/(N-1)
nu_0=solve(VarMat[1:1,1:1])[1,1]/(N-1)
nu_0
nu_1=solve(VarMat[1:2,1:2])[1,1]/(N-1)
nu_1
nu_2=solve(VarMat[1:3,1:3])[1,1]/(N-1)
nu_2
nu_3=solve(VarMat[1:4,1:4])[1,1]/(N-1)
nu_3
...
#I set effect sizes such that MSE1 and MSE2 are equal in theory
sigma2=uniroot(function(s2_funct){
  MSE1_funct=as.double((g1%*%ESvec)^2+nu_1*s2_funct)
  MSE2_funct=as.double((g2%*%ESvec)^2+nu_2*s2_funct)
  MSE1_funct-EffectSizeMSE*MSE2_funct
